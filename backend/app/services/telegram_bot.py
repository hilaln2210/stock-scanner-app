...
async def _cmd_portfolio() -> str:
    try:
        ...
        for ticker, pos in positions.items():
            ...
            lines.append(
                f"{emoji} <b>{ticker}</b> — ${current:.2f} ({pnl_pct:+.1f}%)\n"
...
async def _cmd_brain() -> str:
    try:
        ...
        context = f"תיק: ${equity:.0f} ({pnl_pct:+.1f}%), {len(positions)} פוזיציות."
        if last:
            context += (f"\nהחלטה אחרונה: {last.get('action')} {last.get('ticker', '')} "
                       f"(confidence {last.get('confidence', 0)}%). סיבה: {last.get('reason', '')}.")
        if positions:
            pos_list = ", ".join(f"{t} ({p.get('side', 'long')})" for t, p in positions.items())
            context += f"\nפוזיציות: {pos_list}"
        ...
        return _chat_with_ai(
            "מה המצב? מה החלטת אחרונה? על מה אתה מסתכל עכשיו? תן ניתוח ספציפי של התיק.",
            context
        )
    except Exception as e:
        return f"❌ שגיאה: {e}"
